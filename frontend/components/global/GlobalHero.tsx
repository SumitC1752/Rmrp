        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div>
            ...
            <m.div variants={fadeUp} className="mt-8 flex flex-wrap gap-3">
              {['Middle East', 'North America', 'Europe', 'Asia'].map((region) => (
                <span key={region} className="px-3.5 py-1.5 text-xs font-semibold tracking-wider uppercase rounded-full border border-gold-500/20 text-gold-400 bg-gold-500/5">{region}</span>
              ))}
            </m.div>
          </div>
          ...
          >
            <Image src={IMAGES.worldMap} alt="RMRP Global Trade — Worldwide Export Markets Map" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" priority />
          </m.div>
        </div>
      </m.div>
    </section>
