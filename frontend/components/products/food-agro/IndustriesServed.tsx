...
export default function IndustriesServed() {
  const { badge, heading, subheading, items } = pageContent.industries;
  return (
    <section className="bg-white py-20 sm:py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <m.div variants={container} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="text-center max-w-2xl mx-auto mb-14 lg:mb-18">
          ...
          <m.p variants={fadeUp} className="mt-4 text-base text-gray-500 leading-relaxed">{subheading}</m.p>
        </m.div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {items.map((industry, i) => (
            <m.div key={industry.title} custom={i} variants={cardReveal} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}
            ...
            >
              <h3 className="text-base font-semibold text-navy-900 group-hover:text-gold-600 transition-colors duration-200">{industry.title}</h3>
              <p className="mt-2 text-sm text-gray-500 leading-relaxed">{industry.description}</p>
            </m.div>
          ))}
        </div>
      </div>
