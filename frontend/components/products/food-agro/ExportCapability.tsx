...
export default function ExportCapability() {
  const { badge, heading, description, highlights } = pageContent.exportCapability;
  return (
      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <m.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
            <div className="relative w-full h-full min-h-[340px] lg:min-h-[440px] bg-navy-900 rounded-2xl overflow-hidden">
              <Image src={IMAGES.exportLogistics} alt="Export Logistics & Global Shipping" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" />
            </div>
          </m.div>
          <m.div variants={container} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
            ...
            <m.p variants={fadeUp} className="mt-4 text-base text-gray-600 leading-relaxed">{description}</m.p>
            <div className="mt-8 space-y-3">
              {highlights.map((h, i) => (
                <m.div key={i} custom={i} variants={{ ...fadeUp, visible: { ...fadeUp.visible, transition: { delay: 0.5 + i * 0.08 } } }} initial="hidden" whileInView="visible" viewport={{ once: true }} className="flex items-center gap-3">
