...
export default function ProductOverview({ product }: { product: ProductData }) {
  return (
      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <m.div variants={container} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
            <m.span variants={fadeUp} className="inline-block text-xs font-semibold tracking-widest uppercase text-gold-600 mb-4">Overview</m.span>
            <m.h2 variants={fadeUp} className="text-3xl sm:text-4xl lg:text-5xl font-bold text-navy-900 leading-tight">{product.name}</m.h2>
            {product.overviewParagraphs.map((p, i) => (
              <m.p key={i} variants={fadeUp} className="mt-5 text-base leading-relaxed text-gray-600">{p}</m.p>
            ))}
            <div className="mt-8 space-y-3">
              {product.overviewHighlights.map((h, i) => (
                <m.div
                  key={i}
                  custom={i}
                  variants={{ ...fadeUp, visible: { ...fadeUp.visible, transition: { delay: 0.5 + i * 0.1 } } }}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true }}
